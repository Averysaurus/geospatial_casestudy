...
Okay, this does not look like it has changed much from just looking at the map. We removed about 15% of our calls by only looking at unique places where the event has occured. This approach may be helpful, especially when looking at short time intervals or other kinds of events that have a longer biological or social duration. Now let us try a different approach where we are only interested in points that have identical latitude and longitude coordinates instead. 
...
Questions, comments, concerns? Feel free to contact me here: avery.richards@berkeley.edu
